...
float4 crt_easymode(float2 input_size, float2 output_size, float2 coords)
{
    float2 dx = float2(1.0 / input_size.x, 0.0);
    float2 dy = float2(0.0, 1.0 / input_size.y);
    float2 pix_co = coords * input_size - float2(0.5, 0.5);
    float2 tex_co = (floor(pix_co) + float2(0.5, 0.5)) / input_size;
    ...
    float gamma = GAMMA_INPUT / (DILATION + 1.0);
    ...
    float scan_weight = 1.0 - pow(cos(coords.y * 2.0 * PI * input_size.y) * 0.5 + 0.5, scan_beam) * SCANLINE_STRENGTH;

    float mask = 1.0 - MASK_STRENGTH;    
    float2 mod_fac = floor(coords * output_size * input_size / (input_size * float2(MASK_SIZE, MASK_DOT_HEIGHT * MASK_SIZE)));
    ...
    if (input_size.y >= SCANLINE_CUTOFF) scan_weight = 1.0;
...
float4 PS_CRT_easymode(float2 coords: TEXCOORD0) : COLOR
{
  return crt_easymode(InputSize, OutputSize, coords);
}

technique CRT_easymode
{
    pass Pass0
    {
        PixelShader = compile ps_3_0 PS_CRT_easymode();
    }
}